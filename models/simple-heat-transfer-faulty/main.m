function output_vars = main
  ...
  function [dphidt, extra_output] = f(t, phi) 
    ...
    N_E_110 = [2];
    dH(N_E_110) = E_110(aw, aHnd_x, aHnc_x, aq_y, aHnd_y, aq_x, aHnd_z, aq_z, N_E_110);

    N_E_121 = [2];
    T(N_E_121) = E_121(H, Cp, T_ref, N_E_121);

    N_E_9 = [1];
    T(N_E_9) = E_9(S, U, N_E_9);

    A_E_43 = [1];
    fq_x(A_E_43) = E_43(F, kqA_x, T, Ayz, A_E_43);
...
function sol = E_110(aw, aHnd_x, aHnc_x, aq_y, aHnd_y, aq_x, aHnd_z, aq_z, N)
  sol = aHnc_x(N) + aHnd_x(N) + aHnd_y(N) + aHnd_z(N) + aq_x(N) + aq_y(N) + aq_z(N) + aw(N);
endfunction

function sol = E_121(H, Cp, T_ref, N)
...
function sol = E_43(F, kqA_x, T, Ayz, A)
